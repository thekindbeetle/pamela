...
def get_star(img_x, img_y, start_x, start_y, start_angle_x, start_angle_y,
    ...
    if max(peaks_rate) == 0:
        ...
        if plot_result:
            ...
            fig.suptitle('Event {0}'.format(evnum))
            ax[0].set_title('X')
            ax[1].set_title('Y')
...
calo_events = calo_importer.import_data(os.path.join(dpath, dfile), events=(n_min, n_max + 1))

# Номера событий
event_data = scipy.io.loadmat(os.path.join(dpath, ntrackfile), variable_names=['Event'])['Event'].flatten()
event_idx = (event_data >= n_min) & (event_data <= n_max)
...
for evnum in range(n_min, n_max + 1):
    if evnum not in event_numbers:
        continue

    print('Event {0} is processing'.format(evnum))
    ...
    start_x, start_y, full_angle, start_angle_x, start_angle_y = \
        get_start_direction(img_x, img_y,
                            weight_power=-1.8, shift=0.25, plot_track=True,
                            plot_title='Event {0}'.format(evnum),
                            verbose=True)

    get_star(img_x, img_y, start_x, start_y, start_angle_x, start_angle_y,
             peak_threshold=0.0, peak_min_difference=np.pi / 18, weight_power=-0.3,
             weight_shift=0.25, default_sigma=0.05, max_peaks=100,
             verbose=True, plot=True, plot_result=True, output_file=None)
